	...
	if (connected.length >= 3)
	{
		...
		for (var i = 0; i < connected.length; i++)
		{
			...
			if (idx == idxPup)
			{
				...
				var sprt = cc.Sprite.create("frame#crystals/bomb.png");
				sprt.setPosition(gemX*kGemSize, gemY*kGemSize);
				sprt.setAnchorPoint(0,0);
				sprt.setOpacity(0);
				sprt.runAction(cc.FadeIn.create(0.4));

				var sprtGlow = cc.Sprite.create("frame#crystals/bomb-hi.png");
			...
			else if (idxPup != -1)
			{
				...
				var sprtLight = cc.Sprite.create("frame#crystals/bomb-light.png");
...
function activatePowerUp(x, y)
{
	...
	if (gBoard[idx] == kBoardTypePup0)
	{
		...
		// Horizontal
		var sprtH0 = cc.Sprite.create("frame#crystals/bomb-explo.png");
		...
		// Vertical
		var sprtV0 = cc.Sprite.create("frame#crystals/bomb-explo.png");
		...
		// Horizontal
		var sprtH1 = cc.Sprite.create("frame#crystals/bomb-explo-inner.png");
		...
		// Vertical
		var sprtV1 = cc.Sprite.create("frame#crystals/bomb-explo-inner.png");
...
function setGemType(x, y, newType)
{
	...
	var gemSprite = cc.Sprite.create("frame#crystals/"+newType+".png");
...
function displayHint()
{
	...
	for (var i = 0; i < connected.length; i++)
	{
		...
		var hintSprite = cc.Sprite.create("frame#crystals/hint.png");
...
function setupShimmer()
{
	...
	for (var i = 0; i < 2; i++)
	{
		var sprt = cc.Sprite.create("frame#gamescene/shimmer/bg-shimmer-"+i+".png");
...
function updateSparkle()
{
	...
	sprite = cc.Sprite.create("frame#crystals/sparkle.png");
	...
	if (!gIsGameOver)
	{
		...
		for (x = 0; x < kBoardWidth; x++)
		{
				...
				gTimeSinceAddInColumn[x] >= kTimeBetweenGemAdds)
			{
				...
				var gemSprite = cc.Sprite.create("frame#crystals/"+gemType+".png");
